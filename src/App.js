...
function App() {
  ...
  // Check if name exists in database with proper normalization
  const checkNameExists = () => {
    const normalizedLastName = normalizeString(lastName);
    const normalizedFirstName = normalizeString(firstName);
    const normalizedMiddleName = normalizeString(middleName);
    const normalizedExtension = extension ? extension.toString().trim() : "";

    // Format birthday for comparison (YYYY-MM-DD)
    const searchBirthday =
      year && month && day ? `${year}-${month}-${day}` : null;

    const foundRecord = nameDatabase.find((record) => {
      // Normalize database values for comparison
      const recordLastName = normalizeString(record.lastName);
      const recordFirstName = normalizeString(record.firstName);
      const recordMiddleName = normalizeString(record.middleName);
      const recordExtension = record.extension
        ? record.extension.toString().trim()
        : "";

      // Check if all parts match
      const lastNameMatch = recordLastName === normalizedLastName;
      const firstNameMatch = recordFirstName === normalizedFirstName;

      // Middle name matching - requires exact match
      const middleNameMatch = (() => {
        if (middleName.trim()) {
          return recordMiddleName === normalizedMiddleName;
        } else {
          return !record.middleName || record.middleName.trim() === "";
        }
      })();

      // Extension matching - requires exact match
      const extensionMatch = (() => {
        if (extension) {
          return recordExtension === normalizedExtension;
        } else {
          return !record.extension || record.extension === "";
        }
      })();

      // Birthday matching - requires exact match
      const birthdayMatch = record.birthday === searchBirthday;
